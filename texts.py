...
def announce_broadcast() -> str:
    return (
        "27 июня трое комиков садятся за круглый стол прямо посреди зала. "
        "Без сцены, без дистанции – вы не где-то там в зале, вы в самой гуще.\n\n"
        "<b>Что будет:</b>\n"
        "🎤 Разгоны – комики раскачивают тему, обшучивая её со всех сторон\n"
        "😬 Кринжовые истории – те самые, которые в приличном обществе не рассказывают\n"
        "💬 Темы из зала – вы подкидываете, мы их разгоняем в прямом эфире\n"
        "👀 Секретная рубрика – а вот это узнаете только на месте\n\n"
        "Это пилот – мы обкатываем новый формат и хотим сделать это с вами. "
        "Поэтому вход свободный, а донат – от души, в конце вечера.\n\n"
        "📍 <a href=\"https://maps.app.goo.gl/wF9vGJBekaXh2tYv6\">Грузинский ресторан Kinzadza</a> 🌶\n"
        "🕖 Сбор гостей – 19:00\n"
        "🎤 Начало – 20:00\n"
        "приходите заранее, занять место и заказать что-нибудь вкусное к шоу.\n"
        "🎟 Мест мало – нужна регистрация. Запишитесь по кнопке ниже, и мы придержим вам место.\n\n"
        "Берите компанию – вместе ржать веселее. Количество мест ограничено, "
        "так что не тяните 😉\n\n"
        "До встречи 27-го!"
    )
...
def announce_admin_preview(total_users: int) -> str:
    return (
        ...
        "Уйдёт анонс «Шоу за столом» (фото + текст + кнопка регистрации).\n\n"
        "Кому разослать?"
    )
