...
def greeting_announce() -> str:
    ...
    return _announce_dinner()


def announce_broadcast() -> str:
    """Текст для рассылки анонса всем пользователям (/announce). Тот же, что на /start."""
    return _announce_dinner()


_DINNER_VENUE = (
    "<a href=\"https://maps.app.goo.gl/qzGxjKLiGM8aWwcC8\">"
    "Ресторан «Кинзадза», Нячанг</a>"
)


def _announce_dinner() -> str:
    """Анонс званого ужина «Хинкали и Вино» (4 июля)."""
    return (
        "В Грузии говорят: гость приходит в дом по воле Бога. Поэтому за хорошим столом "
        "всегда найдётся место ещё для одного человека.\n\n"
        "Грузинское застолье вообще не про спешку. Оно про полный стол, наполненные бокалы, "
        "громкий смех, красивые тосты и людей, которым искренне рады.\n"
        "Когда хозяин говорит: «Ешьте. Пробуйте. Наливайте ещё», – это не просто "
        "гостеприимство. Это любовь, которую в Грузии принято выражать через стол.\n\n"
        "Именно такой вечер мы готовим <b>4 июля</b> в Кинзадзе.\n\n"
        "Каждого гостя мы встретим так, как встречают дорогих друзей: четырьмя горячими "
        "хинкали и бокалом грузинского вина. Вместе попробуем разные вкусы и разберёмся, "
        "как всё-таки правильно есть хинкали.\n"
        "И, конечно, ответим на главный вопрос Нячанга: где здесь самые вкусные хинкали?\n\n"
        "Между делом будут тосты, добрые шутки, лёгкие конкурсы, розыгрыши, разговоры и тот "
        "самый момент, когда незнакомые люди вдруг начинают смеяться так, будто знакомы уже "
        "много лет.\n\n"
        "Весь вечер рядом с вами будет <b>Арам Белинский</b>. Он задаст настроение, объединит "
        "гостей за одним столом и сделает так, что этот вечер запомнится не только вкусом "
        "хинкали, но и атмосферой.\n\n"
        "📍 <b>4 июля, 19:00</b>\n"
        "🍷 Званый ужин «Хинкали и Вино»\n"
        f"📍 {_DINNER_VENUE}\n\n"
        "Ждём вас за нашим столом. Приходите голодными. Уйдёте сытыми, весёлыми и, возможно, "
        "с новым любимым тостом.\n\n"
        "👇 Депозит за место – 500 000 ₫ на человека. Регистрируйся, мест немного!"
    )
...
def payment_choice(qty: int, scarcity: str | None, sold_out: bool) -> str:
    ...
    head += "\n👇 Выбери способ оплаты (депозит за 1 человека):"
    return head
...
FEEDBACK_REQUEST = (
    ...
    "Можно текстом, голосовым или кружочком – как душа просит. "
...
def announce_admin_preview(total_users: int) -> str:
    return (
        "📣 <b>Рассылка анонса</b>\n\n"
        f"Всего пользователей бота: <b>{total_users}</b>.\n"
        "Уйдёт анонс «Хинкали и Вино» (фото + текст + кнопка регистрации).\n\n"
        "Кому разослать?"
    )
...
def admin_help() -> str:
    return (
        ...
        "/set_announce – запомнить афишу (ответом на фото)\n"
        "/announce – разослать анонс всем пользователям\n"
        "/broadcast – рассылка по сегментам (текст или ответом на сообщение)\n\n"
        "<b>📝 Посты</b>\n"
        "/addpost – добавить пост в очередь (текст или ответом на фото)\n"
        "/post_now – опубликовать следующий пост из очереди\n\n"
        "<b>🎛 Управление событием</b>\n"
        "/close – закрыть бота для новых гостей («скоро вернёмся»)\n"
        "/open – снова открыть бота\n"
        "/soldout – включить солд-аут · /soldout off – выключить\n"
